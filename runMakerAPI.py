...
def fetch_post_filenames():
    api_url = "https://api.github.com/repos/thenose2003/thenose2003.github.io/contents/_posts"
    response = requests.get(api_url)
    if response.status_code == 200:
        return [item['name'] for item in response.json()]
    else:
        print("Failed to fetch filenames from GitHub.")
        return []
...
"""
    ...
    with open(f"_posts/{datetime.now().strftime("%Y-%m-%d")}-{sanitize_filename(video_title)}", "w", encoding="utf-8") as f:
        f.write(post_md)
...
if __name__ == "__main__":
    youtube_link = "https://youtu.be/rBnLMgRODFw?t=321"
    igns = ["bruhplane", "NoseThe", "Pikachu08t", "Jittarah", "Premeatheouse"]
